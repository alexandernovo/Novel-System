...
$query = "SELECT * FROM donations WHERE donator='$users_id' ORDER BY date_donate DESC";
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div>
            <!-- <?php
                    ...
                    ?>
                <a href="<?php echo $status['users_status'] == 'PENDING' ? '#' : 'add_author_action.php' ?>" class="btn btn-sm px-4 <?php echo $status['users_status'] == 'PENDING' ? 'btn-secondary' : 'btn-primary' ?>"><?php echo $status['users_status'] == 'PENDING' ? 'Already Applied for Author'  : 'Become an Author' ?></a>
            <?php

                    } else { ?>
                <button class="btn btn-success">You are now an Author</button>
            <?php } ?>
            <?php if ($_SESSION['users_role'] != "READER") : ?>
                <a href="profilesettings.php" class="btn btn-dark btn-sm px-4"><i class="fa fa-user"></i> Profile Settings</a>
            <?php endif; ?> -->

            <?php if (isset($_SESSION['userid']) && $_SESSION['users_role'] == "READER") : ?>
                <?php
                $status = first('users', ['users_id' => $_SESSION['userid']]);
                if ($status['users_role'] === "READER") {
                ?>
                    <a href="includes/author_apply.php?apply_author" class="btn btn-sm px-4 btn-primary">Become an Author</a>
                <?php } ?>
            <?php endif; ?>
